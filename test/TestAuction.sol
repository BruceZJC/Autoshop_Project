...
Adoption auction = Adoption(DeployedAddresses.Adoption());
...
function testArrayOfNumericalInformation() public {
    uint[expectedItemCount] memory returnedPriceArray = auction.getAllPrices();
    uint[expectedItemCount] memory returnedPercentArray = auction.getAllIncrease();
    uint[expectedItemCount] memory returnedIncrementArray = auction.getAllIncreament();
...
uint constant expectedItemCount = 10; // expected item count
