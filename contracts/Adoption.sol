...
contract auto_trade{
    ...
    function addItem (string memory vehicle_brand, string memory vehicle_model, string memory description, uint buy_now_price, uint starting_price, uint min_incr, uint offer_price) private { 	
    ...
    function pay_fullprice(uint vehicleId) public returns (uint){
     // taking an integer petId, and return an integer
  ...
  // Function to place a bid
	...
	function getArrayOfPrices () public view returns (uint[itemCount] memory) {
        uint[itemCount] memory resultArray;
        for (uint i=0;i < itemCount; i++) {
            resultArray[i] = this.getItemPrice(i);
        }
		return resultArray;
	}
	...
	function getArrayOfIncreases () public view returns (uint[itemCount] memory) {
		uint[itemCount] memory resultArray;
        for (uint i=0;i < itemCount; i++) {
            resultArray[i] = this.getPercentIncrease(i);
        }
		return resultArray;
	}
	...
	function getArrayOfIncrements () public view returns (uint[itemCount] memory) {
		uint[itemCount] memory resultArray;
        for (uint i=0;i < itemCount; i++) {
            resultArray[i] = this.getItemIncrement(i);
        }
		return resultArray;
	}
